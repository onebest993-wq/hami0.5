-- Fixes for hybrid_search_laws:
-- 1) Strict numeric boundary for article matching (15 should NOT match 115)
-- 2) Enforce unique rows by id in final output
  ...
  SELECT
    ...
    CASE
      WHEN EXISTS (
        ...
        WHERE qn.num <> ''
          AND translate(coalesce(l.article_number, ''), '٠١٢٣٤٥٦٧٨٩', '0123456789')
            ~ ('\\y' || qn.num || '\\y')
      )
      THEN 1000.0
      ELSE 0.0
...
COMMENT ON FUNCTION public.hybrid_search_laws(vector(768), text, integer, double precision)
  IS 'Hybrid search laws with strict numeric boundary and unique id results';
