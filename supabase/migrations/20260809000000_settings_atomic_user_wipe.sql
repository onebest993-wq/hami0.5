-- Atomic server-side wipe for the Settings "مسح كل البيانات" action.
-- The authenticated client cannot execute this function directly; only the BFF
-- service-role may pass the already verified user id.
...
BEGIN
  ...
  -- Legal data and its derived timeline/notification records.
  ...
  -- Settings and calendar deletion tombstones.
  ...
  -- Notification inbox and its append-only user event stream.
  ...
  -- Shares received by or created by this user are no longer visible after wipe.
  DELETE FROM public.case_shares
  WHERE owner_id = p_user_id OR recipient_id = p_user_id;
  GET DIAGNOSTICS sharing_rows = ROW_COUNT;

  -- User-owned forum activity. Moderation/audit records about the user are not
  -- erased (for example forum_bans or reports reviewed by the user).
  ...
  -- Legacy forum rows stored an array of voter ids in the post itself.
  ...
  -- Legacy KV records. This is one server-side statement rather than the old
  -- client O(N) list/get/delete loop.
...
COMMENT ON FUNCTION public.wipe_user_application_data(uuid)
IS 'Atomic deletion of current user application data, invoked only by the authenticated Settings BFF.';
