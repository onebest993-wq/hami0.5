...
-- Strip privileged keys from user-controlled metadata (signup / updateUser)
...
DECLARE
  cleaned jsonb;
  k text;
  allowed text[] := ARRAY['fullName', 'phone', 'avatar_url'];
...
-- Auto-create profile row on signup (always lawyer unless service_role sets profiles.role)
...
-- Forum admin: profiles.role only (not JWT metadata)
...
-- Platform admin for store catalog RLS
...
-- Replace store_products policies that trusted user_metadata
