-- Fix null rows in hybrid_search_laws when article_boost dominates.
-- Root cause: article_number_hits previously returned only id+boost, so FULL OUTER JOIN
-- could yield rows with null law_name/article/content.
...
COMMENT ON FUNCTION public.hybrid_search_laws(vector(768), text, integer, double precision)
  IS 'Hybrid search with exact numeric token boost and non-null article boost rows';
