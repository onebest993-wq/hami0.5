-- O9: UNIQUE الخط الزمني يشمل user_id.
-- على المشروع الحي (hami 0.5) الجدول غير موجود رغم تسجيل 019 —
-- لذلك CREATE TABLE IF NOT EXISTS ثم إسقاط القيد العالمي القديم إن وُجد.
...
ALTER TABLE public.timeline_events
  DROP CONSTRAINT IF EXISTS timeline_events_execution_event_unique;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'timeline_events_user_execution_event_unique'
      AND conrelid = 'public.timeline_events'::regclass
  ) THEN
    ALTER TABLE public.timeline_events
      ADD CONSTRAINT timeline_events_user_execution_event_unique
      UNIQUE (user_id, execution_file_id, event_id);
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_timeline_events_user_execution
  ON public.timeline_events (user_id, execution_file_id);
