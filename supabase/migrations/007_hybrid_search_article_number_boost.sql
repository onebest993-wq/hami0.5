-- Hybrid Retrieval patch:
-- Force article-number dominance when user query contains explicit numbers
-- (e.g. "المادة 31" => rows containing 31 in article_number get +1000 score)
  ...
  SELECT
    ...
    CASE
      WHEN EXISTS (
        ...
        WHERE qn.num <> ''
          AND coalesce(l.article_number, '') ILIKE '%' || qn.num || '%'
      )
      THEN 1000.0
      ELSE 0.0
...
FROM combined c
ORDER BY hybrid_score DESC, similarity DESC NULLS LAST, text_rank DESC NULLS LAST
...
COMMENT ON FUNCTION public.hybrid_search_laws(vector(768), text, integer, double precision)
  IS 'Hybrid search laws via vector similarity + FTS + dynamic keyword + exact article number boost (+1000)';
