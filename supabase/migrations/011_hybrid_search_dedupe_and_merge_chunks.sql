-- Deduplicate hybrid_search_laws by (law_name, article_number)
-- and merge chunked content into one row per article.
...
article_grouped AS (
  SELECT
    ...
    s.law_name,
    s.article_number,
    string_agg(DISTINCT nullif(trim(s.content), ''), E'\n\n' ORDER BY nullif(trim(s.content), '')) AS content,
...
COMMENT ON FUNCTION public.hybrid_search_laws(vector(768), text, integer, double precision)
  IS 'Hybrid search deduped by (law_name, article_number) with merged chunk content and numeric boost';
