...
const handleSaveFastTrack = (data: FastTrackRecord) => {
    ...
    void import('@/app/spark/audit/triggerSparkDocumentAudit').then(
        ({ triggerSparkDocumentAudit, buildLawsuitDossierKey }) => {
            triggerSparkDocumentAudit({
                dossierKey: buildLawsuitDossierKey(
                    String(currentStage.caseNo ?? parentData.caseNo ?? ''),
                    parentData.id ?? 'unknown',
                ),
                fieldType: 'petition',
                text: String(
                    normalized.details ??
                ...
                caseNo: String(currentStage.caseNo ?? parentData.caseNo ?? ''),
                court: String(currentStage.court ?? parentData.court ?? ''),
            });
        },
    );
    SmartToast.success(
...
const handleTransferJurisdiction = (data: { newCourt: string; transferDate: string; notes: string }): boolean => {
    const updatedStages = [...stages];
    const prevStage = updatedStages[activeStageIndex];
...
const handleCourtReferralAcceptance = (data: {
    ...
    setStages((prevStages) => {
        ...
        let prevStage = { ...(updatedStages[activeStageIndex] ?? {}) };
    ...
    if (data.decision === 'reject') {
        ...
        SmartToast.success(`تم قبول المحكمة المحال إليها: ${data.draft?.referredToCourt ?? 'المحكمة الجديدة'}`);
    }
