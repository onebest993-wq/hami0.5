...
import type { RecordPhaseLookupItem } from './casePhaseFilterTypes';
import { caseRecordPhaseShortLabel, journeyNodeRecordPhase } from './casePhaseResolveCore';

export function orderJourneyNodesForLabels(nodes: JourneyNode[]): JourneyNode[] {
    return nodes
...
export function resolveJourneyNodeById(nodeId: string, nodes: JourneyNode[]): JourneyNode | undefined {
    const id = String(nodeId ?? '').trim();
    if (!id) return undefined;
    return nodes.find((n) => n.id === id);
}

export function resolveJourneyNodeByRecordDate(
    itemDate: string,
    nodes: JourneyNode[],
