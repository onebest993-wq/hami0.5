...
export type MovableInlineSectionKey =
    | 'mark'
    | 'experts'
    | 'auction'
    | 'publication'
...
function buildExpertNameSlots(entity: SeizedMovable): string[] {
...
function buildMovableList(
    saveCtx: MovableInlineSaveContext,
    movables: SeizedMovable[],
    movableRow: SeizedMovable,
): SeizedMovable[] {
    return ensureMovableInList(saveCtx.readMovables?.() ?? movables, movableRow);
}
...
export const MovableSeizureInlineSections: React.FC<MovableSeizureInlineSectionsProps> = ({
    ...
    const [markLetter, setMarkLetter] = React.useState(String(m.seizureMarkLetterNumber || ''));
    const [markDate, setMarkDate] = React.useState(String(m.seizureMarkDate || ''));
    const [markEntity, setMarkEntity] = React.useState(String(m.seizureMarkEntity || ''));

    const [expertNames, setExpertNames] = React.useState(
        Array.isArray(m.expertNames) && m.expertNames.length ? m.expertNames.join('، ') : ''
    );
    const [expertNameSlots, setExpertNameSlots] = React.useState(() => buildExpertNameSlots(m));
    const [expertDate, setExpertDate] = React.useState(String(m.expertReportDateYmd || ''));
    const [expertPrice, setExpertPrice] = React.useState(
        m.expertEstimatedAmountIqd != null && Number(m.expertEstimatedAmountIqd) > 0
            ? formatNumberInput(String(m.expertEstimatedAmountIqd))
            : ''
    );

    const [auctionYmd, setAuctionYmd] = React.useState(String(m.auctionDateYmd || m.auction?.auctionDateYmd || ''));

    const [newspaper, setNewspaper] = React.useState(String(m.newspaperName || ''));
    const [pubDate, setPubDate] = React.useState(String(m.publicationDateYmd || ''));

    const [auctionOutcome, setAuctionOutcome] = React.useState<'initial_award' | 'no_bidders'>('initial_award');
    const [buyerName, setBuyerName] = React.useState(String(m.initialAwardBuyerName || ''));
    const [awardAmount, setAwardAmount] = React.useState(
        m.initialAwardAmountIqd != null ? formatNumberInput(String(m.initialAwardAmountIqd)) : ''
    );

    const [reauctionNotes, setReauctionNotes] = React.useState(String(m.reauctionDefault?.notes || ''));

    React.useEffect(() => {
        setMarkLetter(String(m.seizureMarkLetterNumber || ''));
        setMarkDate(String(m.seizureMarkDate || ''));
        setMarkEntity(String(m.seizureMarkEntity || ''));
        setExpertNameSlots(buildExpertNameSlots(m));
        setExpertNames(
            Array.isArray(m.expertNames) && m.expertNames.length ? m.expertNames.join('، ') : ''
        );
    }, [m]);
    ...
    const expertDecision = React.useMemo(() => {
        const subtypes: MovableExpertDecisionSubtype[] = expertDecisionSubtype
            ? [expertDecisionSubtype]
        ...
        return null;
    }, [decisions, movableId, expertDecisionSubtype]);

    const auctionDecision = React.useMemo(() => {
        const row = findSeizureDecisionForMovable(decisions, 'movable_auction_date', movableId);
        if (row && isDecisionResolvedApproved(row) && !String(row.seizureRequestSavedAt || '').trim()) {
            return row;
        }
        return null;
    ...
    const reauctionDecision = React.useMemo(() => {
        const row = findSeizureDecisionForMovable(decisions, 'movable_reauction_default', movableId);
        if (row && isDecisionResolvedApproved(row) && !String(row.seizureRequestSavedAt || '').trim()) {
            return row;
        }
        return null;
    ...
    const requiredExperts = readExpertCommitteeSize(m);
    const hasExpertReportSaved =
        Boolean(String(m.expertReportDateYmd || '').trim()) &&
        m.expertEstimatedAmountIqd != null &&
        Number(m.expertEstimatedAmountIqd) > 0;
                ...
                >
                    <div className="space-y-2 rounded-2xl border border-white/10 bg-black/20 p-3">
                        ...
                        <button
                            ...
                            onClick={() =>
                                saveMovableMarkInline(
                                    buildMovableList(saveCtx, movables, m),
                                    movableId,
