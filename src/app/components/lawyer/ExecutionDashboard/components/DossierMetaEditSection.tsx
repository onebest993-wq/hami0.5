...
export const DossierMetaEditSection: React.FC<DossierMetaEditSectionProps> = ({
    ...
    return (
        <div
            className="fixed inset-0 z-[125] flex items-end justify-center bg-black/62 p-0 sm:items-center sm:p-4 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]"
            dir="rtl"
            onClick={close}
            role="presentation"
        >
            <div
                className="flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border border-amber-500/25 bg-[#0A0F1C] shadow-md sm:rounded-3xl"
            ...
            >
                ...
                <div className="shrink-0 border-t border-white/10 bg-[#0B1120]/95 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
                    <button
                        ...
                        className="inline-flex min-h-[48px] w-full touch-manipulation items-center justify-center rounded-xl bg-amber-700 text-sm font-bold text-amber-50 transition hover:bg-amber-600"
                    >
                        حفظ في ملف الإضبارة
                    </button>
                </div>
