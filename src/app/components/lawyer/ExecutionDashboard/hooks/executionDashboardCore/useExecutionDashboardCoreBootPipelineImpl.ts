...
import { useBootPipelineLinkedChildren } from './useBootPipelineLinkedChildren';
import { resolveExecutionHistoricalMode } from '@/app/utils/executionHistoricalMode';
import { bindExecutionDashboardBootModalAliases } from './bindExecutionDashboardBootModalAliases';

export function useExecutionDashboardCoreBootPipelineImpl({
    file,
    executionId,
    ...
    const inabaTargets = useMemo(() => {
        return allSubFiles
            ...
            .map((f) => ({
                id: f.id,
                directorate: String(f.delegationTargetDirectorate || f.directorate || '').trim() || '---',
            }))
            .filter((x) => x.id);
    ...
    const unifiedTabExecutionData = useExecutionData(
        null,
        (unifiedTabFileRow as ExecutionFile | null | undefined) ?? undefined,
    ...
    const activeFileRow = (isUnifiedTabActive ? unifiedTabFileRow : file) as ExecutionFile | null | undefined;

    const isHistoricalMode = useMemo(
        () =>
            resolveExecutionHistoricalMode({
                dossierLifecycleStatus: executionData?.dossier_lifecycle_status,
                executionArchivedAt:
                    executionData?.executionArchivedAt ?? activeFileRow?.executionArchivedAt,
                executionTrashDeletedAt:
                    executionData?.executionTrashDeletedAt ?? activeFileRow?.executionTrashDeletedAt,
            }),
        [
            executionData?.dossier_lifecycle_status,
            executionData?.executionArchivedAt,
            executionData?.executionTrashDeletedAt,
            activeFileRow?.executionArchivedAt,
            activeFileRow?.executionTrashDeletedAt,
        ],
    );
    ...
    const bootModalAliases = bindExecutionDashboardBootModalAliases(modals, setExecutionModal);
    const {
        showNotesModal,
        setShowNotesModal,
    ...
    const linkedChildren = useBootPipelineLinkedChildren(currentFileId, currentFile);
