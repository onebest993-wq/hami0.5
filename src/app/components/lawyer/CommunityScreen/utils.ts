...
export function deriveTagsFromContent(content: string): string[] {
  ...
  if (/(معامل|معاملات|تجاري|بيع|شراء)/.test(t)) add('معاملات');
  if (/(تقاعد|معاش|راتب_تقاعد|تقاعدي)/.test(t)) add('تقاعد');
  if (/(مصرف|مصارف|بنك|مصرفي)/.test(t)) add('مصارف');
  if (/(قرض|قروض|فائدة|رهن|تمويل)/.test(t)) add('قروض');
  if (/(كاتب_عدل|كاتب العدل|عدل|توثيق|محرر|موثق)/.test(t)) add('كاتب_العدل');
