...
import { getRepositoryMediaIconKind } from './repositoryMedia';
import { VaultPdfJsViewerLazy } from '@/app/components/lawyer/SmartVaultModal/VaultPdfJsViewerLazy';
import { formatRepositoryFileSize } from './repositoryPreviewFileSize';

type RepositoryPreviewDocumentProps = {
...
export function RepositoryPreviewDocument({
    doc,
    signedUrl,
    isLoading,
    isOpenMode,
    ...
    onDownload,
    onOpen,
}: RepositoryPreviewDocumentProps) {
    return (
        <>
            <div className="fixed inset-0 z-[120] bg-black/80" onClick={onClose} />
                ...
                >
                    <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
                        ...
                        <button
                            type="button"
                            onClick={onClose}
                            className="min-h-[44px] min-w-[44px] touch-manipulation rounded-lg bg-white/5 flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-colors"
                        >
                            <X size={18} />
                        </button>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 px-5 py-4 border-t border-white/5">
                        <p className="text-white/30 text-[10px]">تاريخ الرفع: {doc.uploadDate}</p>
                        <div className="flex items-center gap-3">
                            <button
                                type="button"
                                onClick={() => onOpen(doc)}
                                disabled={isLoading || !signedUrl || isOpenMode}
                                className={`min-h-[44px] px-5 py-2.5 rounded-xl text-sm font-bold transition-colors flex items-center gap-2 touch-manipulation ${
                            ...
                            <button
                                type="button"
                                onClick={onClose}
                                className="min-h-[44px] px-5 py-2.5 rounded-xl text-sm text-white/50 hover:text-white transition-colors touch-manipulation"
                            >
                                إغلاق
                            </button>
                            <button
                                ...
                                className="min-h-[44px] px-5 py-2.5 rounded-xl bg-[#E6C673] hover:bg-[#d4b560] text-black text-sm font-bold transition-colors flex items-center gap-2 touch-manipulation"
                            >
                                <Download size={16} />
                                حفظ في الجهاز
                            </button>
