...
import { dispatchOpenEvictionResidentialGrace } from '@/app/utils/openEvictionResidentialGrace';
...
export function ResidentialGraceSection({
    ...
    | 'onResidentialEvictionGraceClick'
>) {
    if (!showResidentialEvictionGraceButton) return null;

    const openGrace = (edit?: boolean) => {
        if (locked) return;
        if (typeof onResidentialEvictionGraceClick === 'function') {
            onResidentialEvictionGraceClick(edit ? { edit: true } : undefined);
        }
        dispatchOpenEvictionResidentialGrace({
            edit: Boolean(edit),
            closeDecisions: true,
        });
    };
    ...
    return (
        <motion.button
            ...
            onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                openGrace(residentialGracePeriodSaved);
            }}
