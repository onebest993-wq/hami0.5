...
export type FatalDeadlinesSectionProps = {
    ...
    onSelectFatalTask?: (taskId: string) => void;
};
    ...
    return (
        ...
        >
            <h2
                ...
                className="font-semibold flex flex-row-reverse items-center gap-2 mb-1 text-sm text-[#F4F4F5]"
            >
                مواعيد قريبة
            ...
            <ul className="divide-y divide-white/[0.06]" role="list">
                {fatalTasks.map((task) => (
                    <li
                        key={task.id}
                        className="first:pt-0 last:pb-0 text-right"
                    >
                        <button
                            type="button"
                            data-testid={`tasks-fatal-summary-${task.id}`}
                            onClick={() => onSelectFatalTask?.(task.id)}
                            className="w-full min-h-[44px] py-2.5 first:pt-2 text-right touch-manipulation"
                            aria-label={`الانتقال إلى ${task.title}`}
                        >
                        <div className="flex flex-row-reverse items-start gap-2.5">
                            ...
                            <div className="min-w-0 flex-1 space-y-1">
                                <p className="text-sm font-semibold text-[#F4F4F5] leading-snug truncate" title={task.title}>
                                    {task.title}
                                </p>
