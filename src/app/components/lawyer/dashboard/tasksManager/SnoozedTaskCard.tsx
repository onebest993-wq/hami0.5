...
export function SnoozedTaskCard({ task, listOrdinal }: SnoozedTaskCardProps) {
    ...
    return (
        <li
            ...
            className={`relative flex flex-row-reverse items-center gap-3 rounded-xl border border-[#A67C52]/18 bg-[#0c0c0e]/55 px-3 py-2.5 min-h-[44px] ${
        ...
        >
            ...
            <div className="min-w-0 flex-1 text-right space-y-0.5">
                <p className="text-sm font-bold text-[#E8F5F0]/90 truncate">{task.title}</p>
                {due ? (
                    <p className="text-[11px] font-semibold text-[#A67C52]/85 flex flex-row-reverse items-center gap-1 justify-end">
