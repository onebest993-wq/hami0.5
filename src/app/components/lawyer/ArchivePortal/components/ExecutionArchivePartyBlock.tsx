...
export function ExecutionArchivePartyBlock({
    ...
    return (
        <div className={className}>
            <div className="flex flex-wrap items-center justify-end gap-2 border-b border-white/6 pb-2">
                <span className="w-full text-[10px] font-medium text-slate-500">الدائن</span>
                {!view.isRepresentingDebtor ? <AgentBadge side="creditor" /> : null}
                <span className="text-sm font-bold text-white">{view.creditorLabel}</span>
            </div>
            <div className="flex flex-wrap items-center justify-end gap-2 pt-0.5">
                <span className="w-full text-[10px] font-medium text-slate-500">المدين</span>
                ...
                <span className="text-sm font-bold text-slate-200">{view.debtorLabel}</span>
            </div>
        </div>
    );
}
