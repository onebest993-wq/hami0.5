...
import { useCaseSharePanel } from '@/app/components/lawyer/NotificationPanel/hooks/useCaseSharePanel';
...
describe('useCaseSharePanel', () => {
    ...
    it('hasContent=false عندما لا توجد مشاركات', () => {
        const { result } = renderHook(() =>
            useCaseSharePanel({ userId: 'user-1', shares: [], onChanged: vi.fn() }),
        );
    ...
    it('respond يستدعي API ويعيد busyId', async () => {
        ...
        const { result } = renderHook(() =>
            useCaseSharePanel({ userId: 'user-1', shares, onChanged }),
        );
    ...
    it('لا يستدعي API إن لم يكن المستخدم مستلم الطلب', async () => {
        const onChanged = vi.fn();
        const shares = [makeShare({ id: 's1', recipientId: 'other', ownerId: 'user-1', status: 'pending' })];
        const { result } = renderHook(() =>
            useCaseSharePanel({ userId: 'user-1', shares, onChanged }),
        );
