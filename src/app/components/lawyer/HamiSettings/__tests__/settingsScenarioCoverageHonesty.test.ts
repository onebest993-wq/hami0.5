...
describe('settings scenario coverage honesty', () => {
    it('E2E الصدفة يغطي الفتح والتبويب والحفظ والهروب والخطر الملغى', () => {
        ...
        expect(shell).not.toContain('settings-toggle-appearance-highContrast');
        expect(shell).not.toContain('settings-font-preset-');
        expect(shell).not.toContain('settings-lite-');
        ...
        expect(shell).not.toContain('settings-toggle-security-privacyBlur');
    ...
    it('E2E الطبقات يغطي الإغلاق والوثيقة والتخصيص والنسخ وإلغاء المسح دون تنفيذ الخطر', () => {
        ...
        expect(scenarios).not.toContain('settings-font-preset-large');
        expect(scenarios).not.toContain('settings-lite-on');
        expect(scenarios).toContain('appearance-chapter-wallpaper');
        expect(scenarios).toContain('settings-wipe-start');
        expect(scenarios).toContain('زر X يغلق الحوار والمركز معاً');
        ...
        expect(scenarios).not.toContain('settings-toggle-security-privacyBlur');
    ...
    it('E2E الموبايل يغطي اللمس و44px والأوراق الأفقية', () => {
        ...
        expect(mobile).toContain('settings-toggle-security-screenshotDeterrent');
        expect(mobile).not.toContain('settings-toggle-security-privacyBlur');
