...
export const REPO_ADD_MENU_PANEL =
    'absolute top-full mt-1 end-0 z-40 min-w-[11rem] rounded-xl border border-white/10 ' +
    'bg-[#121826] p-1 flex flex-col gap-0.5';

export const REPO_ADD_MENU_ITEM =
    'flex w-full items-center gap-2 min-h-[44px] px-2.5 rounded-lg text-xs font-medium text-[#F4F4F5]/90 ' +
    'hover:bg-white/[0.06] hover:text-[#F4F4F5] text-right touch-manipulation';

export const REPO_ROOM_CHIP =
    `${REPO_TOUCH_CHIP} gap-1 px-3 rounded-full text-xs font-medium border-0 transition-colors ` +
...
export const REPO_ROOM_MENU =
    `${REPO_PORTAL_Z} min-w-[14rem] max-w-[min(92vw,17rem)] rounded-xl border border-white/10 ` +
...
export const REPO_INPUT =
    'w-full bg-white/[0.05] border-0 rounded-xl px-3 py-2.5 text-[#F4F4F5] placeholder:text-white/30 outline-none text-base ' +
    'transition-colors focus:bg-white/[0.07] focus:ring-1 focus:ring-[#E6C673]/30';

const REPO_SURFACE_BASE =
    'rounded-xl border border-white/[0.07] bg-white/[0.035]';

/** بطاقة إنشاء المسودة — خفيفة ومضغوطة */
export const REPO_COMPOSE_SHELL =
    `hami-repo-compose ${REPO_SURFACE_BASE} p-2.5 mb-2.5 space-y-2`;
...
export const REPO_COMPOSE_SAVE =
    'inline-flex items-center justify-center gap-1.5 min-h-[44px] px-3.5 rounded-xl text-xs font-medium touch-manipulation ' +
    'bg-[#E6C673] text-[#0A0F1C] hover:bg-[#edd49a] disabled:opacity-50 transition-colors';

export const REPO_COMPOSE_CANCEL =
    'inline-flex items-center justify-center min-h-[44px] px-2.5 rounded-xl text-xs font-medium text-white/45 ' +
...
export const REPO_CARD = `hami-repo-card ${REPO_SURFACE_BASE} p-2.5`;
...
export const REPO_CARD_META =
    'hami-repo-card-meta flex flex-wrap items-center justify-between gap-x-2 gap-y-0.5 mb-1.5 min-h-[1.25rem]';
...
export const REPO_CARD_ACTIONS =
    'hami-repo-card-actions relative z-[2] flex flex-wrap items-center justify-between gap-1 pt-1.5 mt-1.5 border-t border-white/[0.06] pointer-events-auto';

export const REPO_CARD_HEADING = 'text-[11px] font-medium text-[#E6C673]/85 text-right';

export const REPO_BADGE_GOLD = 'hami-repo-badge hami-repo-badge--gold';
export const REPO_BADGE_IMAGE = 'hami-repo-badge hami-repo-badge--image';
export const REPO_BADGE_PDF = 'hami-repo-badge hami-repo-badge--pdf';
export const REPO_BADGE_AUDIO = 'hami-repo-badge hami-repo-badge--audio';
export const REPO_BADGE_FILE = 'hami-repo-badge hami-repo-badge--file';
