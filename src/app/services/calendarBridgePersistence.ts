...
import { normalizeDateToYmd, resolveCalendarUserId } from './calendarBridge';
import type { CalendarSourceModule } from './calendarBridge.types';
import { CALENDAR_SOURCE_PATCHED_EVENT } from './calendarBridge.types';
...
function notifySourcePatched(detail: CalendarSourcePatchDetail): void {
    try {
...
function stripTaskPrefix(sourceEventId: string): string | null {
...
function applyCriminalCalendarUpdate(
...
function applyCriminalCalendarRemoval(
    caseRecord: Record<string, unknown>,
    sourceEventId: string,
): Record<string, unknown> {
    if (sourceEventId === 'location_next_hearing') {
...
async function patchThreadingTaskDeadline(
    userId: string,
...
function isNextUrgentHearingId(sourceEventId: string): boolean {
    return sourceEventId.endsWith('_next');
}

function patchLawsuitStorage(
    fileId: string,
...
function patchExecutionStorage(
    executionId: string,
    mutator: (file: Record<string, unknown>) => Record<string, unknown>,
): boolean {
...
async function patchTransactionStep(
    userId: string,
    transactionId: string,
    stepId: string,
...
function applyLawsuitCalendarUpdate(
...
function applyLawsuitCalendarRemoval(
    file: Record<string, unknown>,
    sourceEventId: string,
    mode: 'soft' | 'hard',
): Record<string, unknown> {
...
function applyExecutionAppointmentUpdate(
    file: Record<string, unknown>,
...
function applyExecutionAppointmentTrash(
    file: Record<string, unknown>,
    timelineEventId: string,
): Record<string, unknown> {
...
async function patchUrgentHearing(
    userId: string,
    caseId: string,
    hearingKey: string,
