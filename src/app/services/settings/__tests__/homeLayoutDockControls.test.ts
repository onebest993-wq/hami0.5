...
describe('homeLayoutDockControls', () => {
    ...
    it('evacuateDockToMain alias keeps quick note in dock', () => {

        const placements = buildDefaultPlacements();

        const { placements: next } = evacuateDockToMain(placements);

        expect(getWidgetsInZone(next, 'dock')).toEqual(['dockQuickNote', 'dockNotepad', 'dockVault']);
