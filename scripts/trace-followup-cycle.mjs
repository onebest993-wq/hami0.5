...
function viteChunk(id) {
  ...
  if (id.includes('ExecutionDashboard/hooks/')) {
    ...
    return 'execution-dashboard-core'; // merged
  }
  return 'other';
}
...
const all = walk(path.join(root, 'app/components/lawyer/ExecutionDashboard'));
const edges = [];

for (const f of all) {
  const from = viteChunk(f);
  if (from !== 'execution-dashboard-core' && from !== 'execution-followup-shared') continue;
  ...
  while ((m = importRe.exec(src))) {
    ...
    if (
      (from === 'execution-dashboard-core' && to === 'execution-followup-shared') ||
      (from === 'execution-followup-shared' && to === 'execution-dashboard-core')
    ) {
      edges.push({ from: path.relative(root, f), to: path.relative(root, dep), fromChunk: from, toChunk: to });
    }
  }
}

console.log('core <-> followup-shared edges:', edges.length);
for (const e of edges) console.log(`${e.fromChunk} -> ${e.toChunk}: ${e.from} -> ${e.to}`);
