#!/usr/bin/env node
/**
 * Royal Profile Tier-1 Production Gate — 4P-STRICT Pattern
 *
 ...
 *   node scripts/profile-production-gate.mjs
 */
...
const EXACT_BANNER = '===== ROYAL PROFILE TIER-1 PRODUCTION GATE PASSED =====';
const EXACT_LAST_STDOUT_LINE = '=== Gate result === PASSED';

const PROFILE_SHADOW_STUB_GLOB_PATHS = [
    'src/app/components/Lawyer/RoyalLawyerProfile/ProfileShieldNucleusSheet.tsx',
    'src/app/SERVICES/profile/profileShieldOwnershipGate.ts',
    'src/app/HOOKS/lawyerDashboard/profile/profileShieldShellShadowOpenFlow.ts',
    'src/app/RUNTIME/storage/encryptedStorage/lawyerProfileShieldBootHydrator.ts',
];
...
const EXCLUDE_PREFIXES = [
    'src/app/components/lawyer/RoyalLawyerProfile/hooks/__tests__/useProfileLoader.test',
];
...
console.log('===== ROYAL PROFILE TIER-1 PRODUCTION GATE =====\n');

// ============================================================
// PHASE 0: Anti-Shadow BOMB — FAIL FAST IF ANY HIT
// ============================================================
banner('PHASE 0: Anti-Shadow BOMB (PROFILE_SHADOW_STUB)');
for (const p of PROFILE_SHADOW_STUB_GLOB_PATHS) {
    const hits = globSync(p, { caseSensitive: false, nodir: true, cwd: PROJECT_ROOT });
    if (hits.length > 0) {
        fail(`BOMB PHASE0 PROFILE_SHADOW_STUB DETECTED: ${p} → hits: ${hits.join(', ')}`);
        console.error('\n===== GATE TERMINATED PHASE0 SHADOW DETECTED =====');
        process.exit(1);
    }
    ok(`phase0 clean: ${p}`);
}
// Additional original shadow check: RoyalLawyerProfile.tsx next to folder
const legacyShadow = globSync('src/app/components/lawyer/RoyalLawyerProfile.tsx', { caseSensitive: false, nodir: true, cwd: PROJECT_ROOT });
if (legacyShadow.length > 0) {
    fail(`BOMB LEGACY SHADOW: RoyalLawyerProfile.tsx shadows /RoyalLawyerProfile/ folder → hits: ${legacyShadow.join(', ')}`);
    console.error('\n===== GATE TERMINATED LEGACY SHADOW DETECTED =====');
    process.exit(1);
}
ok('phase0 clean: no RoyalLawyerProfile.tsx adjacent stub shadow');
...
const criticalGlobs = [
    'src/app/hooks/lawyerDashboard/profile/**/*.{ts,tsx}',
    'src/app/hooks/lawyerDashboard/**/*profile*.{ts,tsx}',
    'src/app/hooks/lawyerDashboard/**/*Profile*.{ts,tsx}',
    'src/app/hooks/**/*useProfile*.{ts,tsx}',
    'src/app/hooks/**/*useLawyerProfile*.{ts,tsx}',
    'src/app/hooks/lawyerDashboard/**/headerShellIntentWarm*.{ts,tsx}',
    'src/app/services/profile/**/*.{ts,tsx}',
    'src/app/services/**/*profileMedia*.{ts,tsx}',
    'src/app/services/cloud/**/*lawyerProfile*.{ts,tsx}',
    'src/app/runtime/*profile*.{ts,tsx}',
    'src/app/runtime/**/*profile*.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/**/*.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/**/__tests__/**/*.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/hooks/**/*.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/components/**/*.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/components/profileTextCanvas/**/*.{ts,tsx}',
    'src/app/components/lawyer/dashboard/profile/**/*.{ts,tsx}',
    'src/app/components/lawyer/dashboard/**/*LawyerDashboardProfileTab*.{ts,tsx}',
    'src/app/components/lawyer/CommunityScreen/components/**/*ForumMemberProfileOverlay*.{ts,tsx}',
    'src/app/components/lawyer/dashboard/forumProfile/**/*ForumTileProfileQuarter*.{ts,tsx}',
    'src/app/runtime/**/*royalLawyerProfile*.{ts,tsx}',
    'src/app/runtime/**/*profileAndroidFx*.{ts,tsx}',
];
...
const testGlobs = [
    'src/app/hooks/lawyerDashboard/__tests__/*profile*.test.{ts,tsx}',
    'src/app/hooks/lawyerDashboard/__tests__/*Profile*.test.{ts,tsx}',
    'src/app/hooks/lawyerDashboard/profile/__tests__/**/*.test.{ts,tsx}',
    'src/app/hooks/__tests__/*useLawyerProfile*.test.{ts,tsx}',
    'src/app/hooks/lawyerDashboard/__tests__/*headerShellIntentWarm*.test.{ts,tsx}',
    'src/app/hooks/lawyerDashboard/__tests__/*lawyerDashboardHeader*.test.{ts,tsx}',
    'src/app/services/profile/__tests__/**/*.test.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/hooks/__tests__/**/*.test.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/components/__tests__/**/*.test.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/components/profileTextCanvas/__tests__/**/*.test.{ts,tsx}',
    'src/app/components/lawyer/RoyalLawyerProfile/__tests__/**/*.test.{ts,tsx}',
    'src/app/components/lawyer/CommunityScreen/components/__tests__/*ForumMemberProfile*.test.{ts,tsx}',
    'src/app/components/lawyer/dashboard/profile/__tests__/**/*.test.{ts,tsx}',
    'src/app/runtime/__tests__/*profile*.test.{ts,tsx}',
    'src/app/runtime/__tests__/*Profile*.test.{ts,tsx}',
    'src/app/runtime/__tests__/*worldclassProfile*.test.{ts,tsx}',
    'src/app/runtime/__tests__/*profileSectionSurgical*.test.{ts,tsx}',
];
...
// ============================================================
if (failed) {
    console.error('\n===== ROYAL PROFILE TIER-1 PRODUCTION GATE FAILED =====');
    console.log('\n=== Gate result ===');
