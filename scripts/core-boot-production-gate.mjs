...
async function main() {
    ...
    try {
        ...
        /**
         * **عيّنةُ إحماءٍ تُهدَر عمداً — وهي تصحيحُ قياسٍ لا تخفيفُ حدّ.**
         *
         * قِيس ٢٠٢٦-٠٩-١٣ على جهازٍ ساكن، والعيّنات الثلاث لسيناريو `desktop`:
         *
         *     694 → 353 → 323 ms   (ttfiMs، متناقصةٌ اطّراداً)
         *     والجوّال بعده: 347 → 320 → 250   (يمرّ مريحاً تحت حدّه)
         *
         * فالأولى **ضِعفُ الثالثة**، والجوّال — وهو يعمل ثانياً على خادمٍ دافئ — يمرّ.
         * أي أنّ العيّنة الأولى تدفع كلفةَ البدء الباردة وحدها: أوّلُ إقلاعٍ للمتصفّح،
         * وأوّلُ ترجمةٍ للوحدات على `vite preview`.
         *
         * **وp95 على ثلاث عيّنات هو الأقصى بحكم الحساب** (`ceil(3×0.95)-1 = 2`)، فكانت
         * البوّابة تُقايس **عيّنةَ البدء الباردة** بميزانيةٍ موضوعةٍ للحالة المستقرّة —
         * **فلا تمرّ على أيّ جهاز**، لا لعطلٍ في المنتج بل لعطلٍ في القياس.
         *
         * فتُهدَر عيّنةٌ واحدة قبل كلّ قياس. **ولا ميزانيةَ تُمسّ ولا إحصاءَ يتغيّر.**
         *
         * **وقِيس الأثر بعد الإصلاح على الجهاز نفسه:**
         *
         *     desktop ttfiMs  قبل: 694 → 353 → 323   p95 = **694**  (تجاوزٌ ٣٤٤)
         *                     بعد: 283 → 366 → 320   p95 = **366**  (تجاوزٌ  ١٦)
         *     mobile  ttfiMs  بعد: 311 → 319 → 323   p95 = 323 < 380  **يمرّ مريحاً**
         *
         * **فزال الأثرُ الصناعيّ وبقي فرقٌ حقيقيّ صغير:** ١٦م.ث على `ttfiMs` و١٩ على
         * `firstTabOpenMs` — أي نحو ٤٪ فوق الحدّ، لا ٩٨٪.
         *
         * **وبقيةُ الفجوة ليست لي:** `percentileSorted(values, 0.95)` يُرجع **الأقصى**
         * لأيّ عدد عيّناتٍ **دون العشرين** (لأنّ `ceil(n × 0.95) = n` عندها؛ وأوّلُ
         * عددٍ ينفكّ عنده هو ٢٠ نفسه)، فزيادةُ العيّنات
         * **تزيد فرصَ عيّنةٍ بطيئة ولا تُنعّم الإحصاء**. فالخياران — تليينُ الإحصاء أو
         * تعديلُ الميزانية — **كلاهما قرارُ مالك لا اجتهادُ منفّذ**، وأخذُ أيٍّ منهما
         * بلا إذنٍ هو «الحلّ الرخيص» الذي يمنعه §٧ المرحلة ٤.
         */
        console.log('\n=== core boot warm-up (discarded — absorbs cold start) ===');
        await run(process.execPath, [
            'scripts/boot-ttfi-audit.mjs',
            `--url=${PREVIEW_URL}`,
            '--device=desktop',
            '--throttle=none',
            '--label=core-boot-warmup-discarded',
        ]);
