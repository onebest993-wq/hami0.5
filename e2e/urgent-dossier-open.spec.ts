...
test.describe('Urgent dossier and reload', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText(/جاري التحميل/i).first()).toBeHidden({ timeout: 25_000 }).catch(() => undefined);
  });
  ...
  test('urgent dossier opens from lawsuits workspace', async ({ page }) => {
    ...
    await expect(page.getByText('مساحة عمل الدعاوى')).toBeVisible({ timeout: 20_000 });
