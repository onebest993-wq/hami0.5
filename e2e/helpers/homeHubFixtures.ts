...
const HOME_HUB_PANEL_SESSION_KEY = 'hami:home-hub-panel';

/** يمنع ترحيل تبويب الهاب من اختبار سابق على نفس الأصل */
export async function clearHomeHubPanelSession(page: Page): Promise<void> {
...
export function buildE2eHubRadarCalendarEvents(count: number) {
    return Array.from({ length: count }, (_, index) => {
        const future = new Date(Date.now() + (2 + index) * 60 * 60 * 1000);
        const { date, time } = baghdadDateTimeParts(future);
        return buildE2eCalendarEvent({
            id: `e2e-radar-event-${index + 1}`,
            title: index === 0 ? 'موعد E2E تجريبي' : `موعد E2E تجريبي ${index + 1}`,
            date,
            time,
        });
    });
