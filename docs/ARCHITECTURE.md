...
## 5️⃣ الذكاء الاصطناعي (AI Architecture)

### نظام Ghost AI

**الهدف:** تحليل ذكي وتنبيهات استباقية
...
if (daysUntilDeadline < 7) {
  ghost.warn('موعد الاستئناف قريب جداً!');
}

if (!case.parties.some(p => p.isClient)) {
  ghost.error('لم يتم تحديد الموكل!');
}
```

### نظام RAG (Retrieval Augmented Generation)

**الهدف:** البحث الذكي في القاعدة القانونية
...
┌─────────────────────────────────────┐
│     Gemini AI                       │
│   صياغة إجابة من المستندات          │
└───────────────┬─────────────────────┘
                │
                ▼
┌─────────────────────────────────────┐
│     Response to User                │
│   إجابة دقيقة مع المصادر            │
└─────────────────────────────────────┘
```
