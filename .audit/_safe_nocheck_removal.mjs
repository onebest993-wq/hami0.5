...
for (const f of headHad) addNc(f);
console.log('Reset complete, headHad', headHad.size);
...
const preferRe =
  /(pick|Static|Bridge|Bags|collectScope|Fragment|PipelineInput|RuntimeVarKeys|badgeSort|toneRing|hiddenBadge|submitCoreTypes|PanelModel|DebtorEmployment|PushSeizureAuction|CoerciveUiState|GracePeriodEnd|NotesOverlays|SolidaryEviction|LightHandlers|CoreRuntimeVars|ScopeLocal|ScopeRest|HandlerClusterLight\.ts|HandlerClusterFoundation\.ts|FoundationTimeline|PushTimelineEvent)/i;
...
// Prefer preferRe first
pool = [
  ...pool.filter((f) => preferRe.test(f)),
  ...pool.filter((f) => !preferRe.test(f)),
];
...
// Take first 40 from pool for bisect
const target = pool.slice(0, 40);
const removed = bisect(target);

const nowNc = all.filter((f) => hasNc(fs.readFileSync(f, 'utf8'))).length;
console.log(JSON.stringify({ removed: removed.length, remainingNocheck: nowNc, headHad: headHad.size }, null, 2));
